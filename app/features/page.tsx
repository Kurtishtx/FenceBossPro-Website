  ...
  return (
    <>
      ...
      <div className="hero">
        ...
        <h1>Everything You Need to Run<br /><span>Your Fence Business</span></h1>
        <p>FenceBossPro is built specifically for fence contractors. Every feature listed here is included in your from $59/month — nothing locked, nothing extra.</p>
      ...
      <div className="spotlight">
        <div className="spotlight-inner">
          <div className="spotlight-text">
            <div className="spotlight-label">Only in FenceBossPro</div>
            <div className="spotlight-title">Circle an Area on the Map.<br /><span>Instantly Know Everything Inside It.</span></div>
            <div className="spotlight-desc">On the job board map, draw a circle around any geographic area. FenceBossPro instantly shows you the total linear feet, job count, number of jobs by type, and a breakdown of every fence type inside that circle — before you dispatch a single crew.</div>
            <ul className="spotlight-checks">
              <li>Draw a lasso around any area on the map</li>
              <li>See total linear feet for every job inside</li>
              <li>Total jobs and total work in the selected area</li>
              <li>Breakdown by fence type with totals for each</li>
              <li>Dispatch all circled jobs with one click</li>
              <li>They drop straight onto the dispatch board with a full route map</li>
              <li>No other fencing software does this</li>
            </ul>
          </div>
          <div className="spotlight-visual">
            ...
            <div className="stat-row">
              ...
              <div className="stat-box"><div className="stat-val">19</div><div className="stat-lbl">Total Jobs</div></div>
              <div className="stat-box"><div className="stat-val">8,600</div><div className="stat-lbl">Linear Ft</div></div>
              <div className="stat-box"><div className="stat-val">22</div><div className="stat-lbl">Gates</div></div>
              <div className="stat-box wide"><div className="stat-val">Wood · 8 &nbsp;|&nbsp; Vinyl · 6 &nbsp;|&nbsp; Chain Link · 5</div><div className="stat-lbl">Jobs by Fence Type</div></div>
            </div>
            <button style={{width:'100%',marginTop:'14px',background:'var(--orange)',color:'#fff',border:'none',borderRadius:'8px',padding:'13px',fontSize:'14px',fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}
              onClick={(e) => openSignupModal(1, e.currentTarget as HTMLElement)}>Dispatch These 14 Jobs →</button>
          </div>
        </div>
      </div>
      ...
      <section style={{background:'var(--light-bg)'}}>
        <div className="cat-block">
          <div className="cat-header">
            ...
            <div><div className="cat-title">Scheduling &amp; Dispatch</div><div className="cat-desc">Build routes, assign jobs, and manage your full day from one board</div></div>
          </div>
          <div className="feat-grid">
            {[
              {t:'Linear-Foot Job Board by Fence Type',d:'See total linear feet waiting for each fence type before you build a route. Know what you have before you make a call.'},
              {t:'Circle-to-Dispatch Map Tool',d:'Draw a circle on the job board map. Get instant totals — linear feet, jobs, fence types — then dispatch them all at once.'},
              {t:'One-Click Schedule from Job Board',d:'Click any job and assign a date, crew, and truck in seconds. No extra screens, no copy-paste.'},
              {t:'Daily Dispatch Board',d:'All scheduled jobs for any day in one view. Filter by crew, truck, or fence type. Print dispatch sheets for drivers.'},
              ...
              {t:'Day Summary Bar',d:'Total jobs, total linear feet, and total revenue for the day shown at the top of the dispatch board at a glance.'},
              {t:'Full Job History per Property',d:'Every completed job logged to the property record with date, crew, notes, and materials used.'},
            ].map((f,i) => (
      ...
      <section>
        <div className="cat-block">
          ...
          <div className="feat-grid">
            {[
              {t:'Live Route Map',d:'Every scheduled job for the day pinned on an interactive map. See your full route before any crew leaves the shop.'},
              {t:'Drag-and-Drop Job Reordering',d:'Reorder jobs on the map or dispatch board. Build tight geographic clusters to cut windshield time.'},
      ...
      <div className="dark-cat">
        <div className="cat-block" style={{maxWidth:'1200px',margin:'0 auto'}}>
          <div className="cat-header">
            <span className="cat-icon">🪵</span>
            <div><div className="cat-title">Materials, Parts &amp; Takeoffs</div><div className="cat-desc">Price every job by the linear foot with posts, panels, pickets, rails, concrete, gates, and hardware built in</div></div>
          </div>
          <div className="feat-grid">
            {[
              {t:'Linear-Foot Takeoffs',d:'Enter the run length and FenceBossPro calculates posts, panels, pickets, rails, and concrete for the job — automatically.'},
              {t:'Materials & Parts Catalog',d:'Build your library of posts, panels, pickets, rails, concrete, gates, and hardware once. Pull from it on every bid.'},
              {t:'Per-Fence-Type Material Lists',d:'Wood, vinyl, chain link, aluminum, or ornamental — each fence type pulls its own material list and pricing.'},
              {t:'Gate & Hardware Line Items',d:'Add walk gates, drive gates, hinges, latches, and post caps as their own line items on any estimate.'},
              {t:'Markup & Cost Tracking',d:'Set your cost and markup per material. FenceBossPro tracks margin on every job so you never bid below cost.'},
              {t:'Material Summary per Job',d:'Generate a full materials pull-list for any job — ready for the supply yard before your crew loads the truck.'},
            ].map((f,i) => (
              <div key={i} className="feat-item"><div className="feat-item-title">{f.t}</div><p>{f.d}</p></div>
            ))}
          </div>
      ...
      <section style={{background:'var(--light-bg)'}}>
        <div className="cat-block">
          <div className="cat-header">
            <span className="cat-icon">💰</span>
            <div><div className="cat-title">Estimates &amp; Bids</div><div className="cat-desc">Build, send, and close line-item fence bids without leaving FenceBossPro</div></div>
          </div>
          <div className="feat-grid">
            {[
              {t:'Line-Item Bid Builder',d:'Build your materials and labor catalog once. Add line items to any bid by selecting from your list — no retyping prices.'},
              {t:'Email Estimate Directly',d:'Send a branded fence bid to the client in one click. No downloading, no copy-paste into Gmail.'},
              {t:'Online Accept Link',d:'Clients click a link to view and accept. Accepted status updates in your dashboard immediately.'},
              {t:'3-Step Automated Follow-Up',d:"3 follow-up texts go out automatically if a client doesn't respond. FenceBossPro chases the deal so you don't have to."},
              {t:'Discount Codes & Sales Tax',d:'Apply percentage or flat discounts. Set tax rates and FenceBossPro calculates tax on every bid automatically.'},
              {t:'Estimate Status Tracking',d:'Track every bid as Draft, Sent, Accepted, or Declined with full history and timestamps.'},
            ].map((f,i) => (
              <div key={i} className="feat-item"><div className="feat-item-title">{f.t}</div><p>{f.d}</p></div>
            ))}
          </div>
      ...
      <section>
        <div className="cat-block">
          ...
          <div className="feat-grid">
            {[
              {t:'Convert Bid to Invoice',d:'Turn any accepted bid into an invoice in one click. Line items, discounts, and tax carry over automatically.'},
              {t:'Deposits & Progress Billing',d:'Collect a deposit up front, then bill progress payments as the fence goes up. Track what is paid and what remains.'},
              {t:'Stripe Card-on-File',d:'Store customer cards securely via Stripe. Charge on completion without the customer needing to do anything.'},
      ...
      <div className="dark-cat">
        <div className="cat-block" style={{maxWidth:'1200px',margin:'0 auto'}}>
          ...
          <div className="feat-grid">
            {[
              {t:'Two-Way SMS Inbox',d:'Send and receive texts with customers inside FenceBossPro. Full conversation history organized by contact.'},
              {t:'Job Scheduled Alert',d:'Customer gets a text the moment their fence job is added to the schedule. Fully customizable message.'},
              {t:'Crew On the Way Alert',d:'Notify customers automatically when your crew is heading to their property.'},
              {t:'Job Completed Alert',d:'Text the customer when the fence is marked done — what was installed, no phone call needed.'},
              {t:'Job Skipped Alert',d:"If a job is skipped, an alert goes out automatically so the customer isn't left in the dark."},
              {t:'Bid Sent & Accepted Alerts',d:'Notify clients when their fence bid is on the way, and get notified yourself when they accept.'},
              {t:'Google Review Request',d:'Automatically sent after every completed job. More 5-star reviews with zero extra effort.'},
              ...
              {t:'500 Texts/Month Included (US & Canada)',d:'500 texts a month — sent and received — included in the from $59/month. Add more in blocks of 500 for 1295 each.'},
      ...
      <section style={{background:'var(--light-bg)'}}>
        <div className="cat-block">
          <div className="cat-header">
            <span className="cat-icon">🏠</span>
            <div><div className="cat-title">Clients, Leads &amp; Properties</div><div className="cat-desc">Full CRM built for fence contractors — everything tied to the address, not just the name</div></div>
          </div>
          <div className="feat-grid">
            {[
              {t:'Client Database',d:'Full searchable database of every customer — contact info, notes, job history, invoices, and bids in one place.'},
              ...
              {t:'Property Profiles',d:'Every address has its own record — fence linear footage, GPS, photos, job history, and notes.'},
              {t:'Multiple Properties per Client',d:'One client can own multiple job addresses. Each property is tracked independently with its own full history.'},
              {t:'Property Photos & Notes',d:'Attach photos and field notes to any property. Crew notes from the field show up in the office view instantly.'},
      ...
      <section>
        <div className="cat-block">
          <div className="cat-header">
            <span className="cat-icon">📦</span>
            <div><div className="cat-title">Multi-Phase Projects &amp; Warranties</div><div className="cat-desc">Manage staged fence builds and never miss a warranty callback</div></div>
          </div>
          <div className="feat-grid">
            {[
              {t:'Custom Project Phases',d:'Break any fence build into phases — demo, post-setting, panel install, gates, final walk. Fully custom phase types and counts.'},
              {t:'Assign Clients to Projects',d:'Link any client to any project. Track start date, phases completed, and phases remaining automatically.'},
              {t:'Warranty Callback Alerts',d:'Get alerted before a fence warranty window closes so you can follow up before the client looks elsewhere.'},
              {t:'Phase Count Tracking',d:'FenceBossPro counts down remaining phases automatically each time a stage is marked complete.'},
            ].map((f,i) => (
              <div key={i} className="feat-item"><div className="feat-item-title">{f.t}</div><p>{f.d}</p></div>
            ))}
          </div>
      ...
      <div className="dark-cat">
        <div className="cat-block" style={{maxWidth:'1200px',margin:'0 auto'}}>
          ...
          <div className="feat-grid">
            {[
              {t:'Unlimited Users',d:'Add every employee — installers, office staff, managers — at no extra cost. No per-user fees, ever.'},
              {t:'Role-Based Permissions',d:'Owner, Manager, Office, Installer, and Mobile roles — each with different access levels built in.'},
              ...
              {t:'Truck Management',d:'Create vehicle profiles, assign trucks to routes, and track which vehicle handled each job.'},
      ...
      <section style={{background:'var(--light-bg)'}}>
        <div className="cat-block">
          <div className="cat-header">
            <span className="cat-icon">📱</span>
            <div><div className="cat-title">Mobile App for Crews</div><div className="cat-desc">Field-optimized for your installers — no office calls, no app store required</div></div>
          </div>
          <div className="feat-grid">
            {[
              ...
              {t:'Material Log Entry',d:'Crews log the posts, panels, and hardware used at each job from their phone. Records are automatic.'},
              ...
              {t:'No App Download Required',d:"FenceBossPro's mobile experience runs in any browser. No app store, no install, no update prompts."},
            ].map((f,i) => (
              <div key={i} className="feat-item"><div className="feat-item-title">{f.t}</div><p>{f.d}</p></div>
            ))}
          </div>
        ...
        <div className="cat-block">
          ...
          <div className="feat-grid">
            {[
              ...
              {t:'Materials & Job Reports',d:'Pull a print-ready materials and job report for any date range, fence type, property, or crew in seconds.'},
      ...
      {/* FORM 1 */}
      <div id="sbp-form-1" style={{display:'none',position:'fixed',zIndex:99999,width:'420px',maxWidth:'calc(100vw - 24px)',background:'#fff',borderRadius:'14px',border:'3px solid #2b6ca3',boxShadow:'0 0 0 4px rgba(43,108,163,.35), 0 16px 60px rgba(0,0,0,.45)',maxHeight:'calc(100vh - 40px)',overflowY:'auto'}}>
        ...
        <div id="sbp1-step2" style={{padding:'24px 28px',display:'none'}}>
          ...
          <div style={{marginBottom:'18px',display:'flex',alignItems:'flex-start',gap:'10px'}}>
            <input type="checkbox" id="sbp1-agree" style={{width:'16px',height:'16px',accentColor:'#2b6ca3',cursor:'pointer',flexShrink:0,marginTop:'3px'}} />
            <label htmlFor="sbp1-agree" style={{fontSize:'13px',color:'#555',cursor:'pointer',lineHeight:1.5}}>I agree to the <a href="https://fencebosspro.com/terms" target="_blank" rel="noreferrer" style={{color:'#2b6ca3'}}>Terms of Service</a> and <a href="https://fencebosspro.com/privacy-policy" target="_blank" rel="noreferrer" style={{color:'#2b6ca3'}}>Privacy Policy</a></label>
          </div>
          <button id="sbp1-create-btn" onClick={() => sbpCreateAccount(1)} style={{width:'100%',background:'#2b6ca3',color:'#fff',border:'none',borderRadius:'6px',padding:'13px',fontSize:'15px',fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>Create My Account</button>
      ...
      {/* FORM 2 */}
      <div id="sbp-form-2" style={{display:'none',position:'fixed',zIndex:99999,width:'420px',maxWidth:'calc(100vw - 24px)',background:'#fff',borderRadius:'14px',border:'3px solid #2b6ca3',boxShadow:'0 0 0 4px rgba(43,108,163,.35), 0 16px 60px rgba(0,0,0,.45)',maxHeight:'calc(100vh - 40px)',overflowY:'auto'}}>
        <div style={{background:'linear-gradient(135deg,#0f1720,#1f2d3d)',padding:'28px 28px 22px',position:'relative'}}>
          <div style={{color:'#fff',fontSize:'20px',fontWeight:800,paddingRight:'36px'}}>Start Your 14-Day Free Trial</div>
          <div style={{color:'rgba(255,255,255,.6)',fontSize:'13px',marginTop:'5px'}}>No credit card required · Full access · from $59/mo after your 14-day trial</div>
        ...
        <div id="sbp2-step2" style={{padding:'24px 28px',display:'none'}}>
          ...
          <div style={{marginBottom:'18px',display:'flex',alignItems:'flex-start',gap:'10px'}}>
            <input type="checkbox" id="sbp2-agree" style={{width:'16px',height:'16px',accentColor:'#2b6ca3',cursor:'pointer',flexShrink:0,marginTop:'3px'}} />
            <label htmlFor="sbp2-agree" style={{fontSize:'13px',color:'#555',cursor:'pointer',lineHeight:1.5}}>I agree to the <a href="https://fencebosspro.com/terms" target="_blank" rel="noreferrer" style={{color:'#2b6ca3'}}>Terms of Service</a> and <a href="https://fencebosspro.com/privacy-policy" target="_blank" rel="noreferrer" style={{color:'#2b6ca3'}}>Privacy Policy</a></label>
          </div>
          <button id="sbp2-create-btn" onClick={() => sbpCreateAccount(2)} style={{width:'100%',background:'#2b6ca3',color:'#fff',border:'none',borderRadius:'6px',padding:'13px',fontSize:'15px',fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>Create My Account</button>
      ...
      <style>{`
        .spotlight { background: linear-gradient(135deg, #0f1720 0%, #16202b 100%); padding: 90px 40px; }
        ...
        .spotlight-visual { flex: 1; min-width: 280px; background: rgba(255,255,255,.05); border: 2px solid rgba(43,108,163,.35); border-radius: 16px; padding: 32px 28px; }
        ...
        .mock-pin.sel { background: var(--orange); box-shadow: 0 0 0 3px rgba(43,108,163,.3); }
        ...
        .feat-item:hover { border-color: var(--orange); box-shadow: 0 4px 20px rgba(43,108,163,.1); transform: translateY(-2px); }
        ...
        .dark-cat { background: linear-gradient(135deg, #0f1720 0%, #16202b 100%); padding: 80px 40px; }
